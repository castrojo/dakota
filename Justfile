...
# Output: sbom.json in repo root.
#
# Local testing:
#   just sbom                          # generate SBOM
#   jq '.spdxVersion' sbom.json        # verify SPDX-2.3
#   jq '.packages | length' sbom.json  # expect ~1100+
#   jq -r '.packages[].name' sbom.json | grep -i "gnome\|gtk\|systemd"
[group('test')]
sbom:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    podman run --rm \
        ...
        "
    echo ""
    echo "==> SBOM written to: $(pwd)/sbom.json"
    du -sh sbom.json
    echo ""
    echo "==> Package count:"
...
verify image_ref="":
    ...
    if ! command -v cosign &>/dev/null; then
        ...
        cosign verify \
            --certificate-identity-regexp \
                'https://github.com/projectbluefin/dakota/.github/workflows/build.yml@refs/heads/main' \
            --certificate-oidc-issuer https://token.actions.githubusercontent.com \
            "${IMAGE}" && echo "PASS: signature valid" || echo "FAIL: signature check failed"
    fi
    echo ""
    ...
    if ! command -v oras &>/dev/null; then
        ...
        oras discover "${IMAGE}" && echo "PASS: referrers listed above" || echo "FAIL: oras discover failed"
    fi
    echo ""
    ...
    if ! command -v gh &>/dev/null; then
        ...
        gh attestation verify "oci://${IMAGE}" \
            --repo projectbluefin/dakota && echo "PASS: attestation valid" || echo "FAIL: attestation check failed"
    fi
