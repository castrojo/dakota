...
| **Linker cache** | **`ldconfig -r /layer -f /layer/etc/ld.so.conf`** | **Rebuilds `/etc/ld.so.cache` for the sysroot. Without this, any library SO version bump leaves the deployed system with a stale linker cache after `bootc switch`, causing `dlopen()` failures on first boot.** |
...
`ldconfig -r /layer -f /layer/etc/ld.so.conf` in the image build writes a correct cache into the image's
...
-r /layer -f /layer/etc/ld.so.conf` step in the build to prevent recurrence.

> If you are adding a new post-install step to `elements/oci/bluefin.bst`, insert
> it **before** `ldconfig -r /layer -f /layer/etc/ld.so.conf` so the linker cache reflects the final
> installed state.
