...
|---|---|---|
| Build OOM or hangs | Memory pressure with 4 builders | Check element build resource usage; for GCC/bootstrap ICEs or OOM crashes (e.g. GCC 15 toolchain boots on massive autogenerated files like gimple-match.cc), override `common_flags` in `elements/freedesktop-sdk.bst` to `-O1 -pipe -Wno-error` to stabilize compilation, and set `build.max-jobs: 1` in the CI config to prevent memory exhaustion while allowing `scheduler.builders: 2` for parallel element scheduling. |
